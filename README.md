...
页面始终显示两种分卷输出方式；浏览器不支持文件夹写入时，“选择文件夹”会置灰并显示原因：

| 分卷输出方式 | 保存行为 | 主要内存占用 |
|--------------|----------|----------------|
| 逐卷下载 | 非流式下载；每卷生成后手动确认，保存后继续下一卷 | 一个分卷及处理缓冲 |
| 选择文件夹 | 自动保存全部分卷，无需逐卷确认 | 一个分卷及处理缓冲 |

普通“解密”页面也会显示输出方式。支持 File System Access API 的浏览器可以逐个流式保存或选择文件夹，内存主要由所选处理块大小决定；不支持时只能使用“浏览器下载”，完整解密结果会累积在内存中，内存占用约为整个文件大小及额外浏览器缓冲。

普通加密和解密页面中的“处理块大小”只在流式写入模式下可调整。受限浏览器会固定使用 10 MB 处理块并禁用选择器，因为减小单次处理块不能减少完整输出的总内存。受限模式解密超过 2 GB 的文件时，页面会在开始处理前建议转到“转压缩包”，并可直接带入文件、算法和密码。
